...
const TEAM_CODE_MAP = {
  삼성: "SAMSUNG", 키움: "KIWOOM", LG: "LG", 두산: "DOOSAN", KIA: "KIA",
  NC: "NC", 한화: "HANWHA", SSG: "SSG", KT: "KT", 롯데: "LOTTE",
};
...
const toTeamCode = (name) => {
  ...
  return Object.entries(TEAM_CODE_MAP).find(([k]) => trimmed.includes(k))?.[1] ?? trimmed;
};

const parseDate = (text, season) => {
  const m = text.match(/(\d+)\.(\d+)/);
  return m ? `${season}-${m[1].padStart(2, "0")}-${m[2].padStart(2, "0")}` : null;
};
...
const parseTeams = async (playCell) => {
  ...
  if (spans.length >= 2) {
    return { away: toTeamCode(spans[0]), home: toTeamCode(spans[spans.length - 1]) };
  }
  const parts = (await playCell.innerText()).replace(/\d+/g, "").split("vs").map((s) => s.trim());
...
const parseRow = async (row, season, seriesValue, state) => {
  if ((await row.locator("td.play").count()) === 0) { return null; }

  if ((await row.locator("td.day").count()) > 0) {
    state.date = parseDate(await row.locator("td.day").innerText(), season) ?? state.date;
  }
